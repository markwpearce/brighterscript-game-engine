...
- `SceneObject` subclasses (`sceneObjects/SceneObject*.bs`) represent what actually gets drawn: `SceneObjectImage`, `SceneObjectBillboard`, `SceneObjectLine`, `SceneObjectPolygon`, `SceneObjectText`, `SceneObjectModel`, `SceneObjectPlane`. Each `SceneObjectDrawMode` (`matchCamera`, `directToCamera`, `directScaled`, `oriented`/`orientedDrawBackFace`, `wireFrame`/`wireFrameDrawBackFace`, `solid`/`solidDrawBackFace`) controls how an object reacts to camera rotation/perspective — this is what gives the 2D-renderer-based engine its pseudo-3D/billboard capability (see `examples/3d`).
