...
- `Renderer.drawFogBands(fogColorRGB, fogEndY, bandCount, bandAlpha)` is a cheap screen-space distance-fog approximation — it stacks `bandCount` translucent, full-width rectangles all anchored at the canvas top (y=0) with decreasing height, so overlap (and therefore opacity) builds up near the horizon and thins out toward `fogEndY`, using nothing but ordinary `DrawRect` calls. It's purely screen-space (no camera-distance or per-object math), so it only reads as fog for a horizon-style scene where screen Y already correlates with distance — see its use in `examples/terrain`'s `MainRoom.onDrawEnd()`. Not a substitute for real per-object depth fog in scenes where distant objects can appear anywhere on screen.
