...
- **Input, while `style = popup`:**
  ...
  - `handleInput()`: while `m.expanded`, Up/Down (throttled via the existing `RepeatThrottle`) move `highlightedIndex` (clamped, no wrap), `back`-press cancels (`m.expanded = false`, no change) — both consume the event. While *not* expanded, `style = popup` disables the existing Left/Right inline-cycling (popup is the only interaction path once that style is chosen) and falls through to normal focus navigation.
  - **Correction (confirmed during Task 3 implementation):** OK-press never reaches `handleInput()` at all — `FocusManager.update()` always intercepts OK-press itself and calls `onMouseDown()`/`onClick()` directly (see its `update()`). So the commit step lives in `onClick()`, not `handleInput()`: `onClick()` toggles — first click while `not m.expanded` expands (seeding `highlightedIndex` from `selectedIndex`, as above); second click while `m.expanded` commits (`m.selectedIndex = m.highlightedIndex`, fires `onChanged`, `m.expanded = false`). `handlePopupInput()` (the `handleInput()` helper) only ever handles Up/Down and back.
  - Because `handleInput()` consumes and returns `true` for Up/Down/back while `m.expanded`, `FocusManager.update()`'s "only navigate if not handled" rule already keeps focus from moving away mid-popup — no `FocusManager` changes needed there.
