...
Per-frame flow:
1. `onUpdate(dt)`: snapshot `m.positionBeforeMove = clone(m.position)` *before*
   ...
   Handle input: horizontal run speed, jump (on press, if grounded or within
   coyote time; buffer a jump press briefly if airborne so a press just
   before landing still fires), variable jump height by cutting `velocity.y`
   if the button releases early. Decrement `grounded` unless refreshed this
   frame by `onCollision` (see below) — i.e. `grounded` starts each frame
   assumed false and `onCollision` sets it true when applicable.
2. Engine integrates `position += velocity * dt`.
3. `onCollision(myCollider, otherCollider, otherEntity)`: only acts when
...
it). On colliding with `Player`: completes the level (a simple "you win" UI
state via `gameUi`, no further scope than that).
...
- Owns a `gameUi` HUD (score, lives) and the win-state UI.
